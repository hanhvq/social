...
public class RelationshipAction {
  ...
  public Response.Content index(Request request) {
    return index.ok(parameters(Context.getInstance(request, bundle)));
  }
  ...
  public enum ActionListener {
    ...
    public Response execute(Context context, Template template) throws Exception {
      ...
      if (isValid()) {
        ...
        //
        AppendableStream as = new AppendableStream(Tools.UTF_8, buffer);
        template.renderTo(as, parameters(context));
